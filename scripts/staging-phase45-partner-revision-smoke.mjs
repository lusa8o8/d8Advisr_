...
const owned=await req(`/rest/v1/venues?select=id,name,category,address,area,description,open_hours,cover_image,images,updated_at,listing_status&partner_id=eq.${partner.id}&order=created_at.desc&limit=1`,partner.token);
assert(owned.r.ok&&owned.b?.[0]?.listing_status==='live','Staging partner needs one live owned venue');
...
try{
 ...
 assert(rejected.r.ok&&rejected.b.status==='rejected','admin rejection failed');
 ...
 const pending=(await req(`/rest/v1/venues?select=name,category,address,area,description,open_hours,cover_image,images&id=eq.${venueId}`,consumer.token)).b[0];
 ...
 const after=(await req(`/rest/v1/venues?select=name,category,address,area,description,open_hours,cover_image,images,listing_status,verification_status&id=eq.${venueId}`,consumer.token)).b[0];
 ...
 assert(approvalAudit.r.ok&&approvalAudit.b.length>=6,'approved partner high-risk fields were not audited');
 ...
 if(temporaryApproved){
  ...
  const final=(await req(`/rest/v1/venues?select=name,category,address,area,description,open_hours,cover_image,images&id=eq.${venueId}`,partner.token)).b[0];
  for(const field of ['name','category','address','area','description','cover_image']){
   assert(JSON.stringify(final[field])===JSON.stringify(before[field]),`restoration mismatch: ${field}`);
  }
  assert(JSON.stringify(final.open_hours)===JSON.stringify(before.open_hours)&&JSON.stringify(final.images)===JSON.stringify(before.images),'restoration mismatch: structured fields');

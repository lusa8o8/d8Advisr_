...
export function HomeDiscovery() {
  ...
  // Discovery is keyed by the canonical D8 market ID, never display-city text.
            ...
            <div className="grid grid-cols-3 gap-4">
              {eventsLoading
                ...
                : EXPERIENCES.length === 0 ? (
                    <div className="col-span-3 bg-white rounded-2xl border border-dashed border-gray-200 p-8 text-center">
                      <p className="font-bold text-gray-800 text-[15px]">
                        {eventsError ? 'Events could not load' : 'No upcoming events yet'}
                      </p>
                      <p className="text-[12px] text-gray-400 mt-1">
                        {eventsError ? eventsError : 'Live events will appear here once their start dates are in the future.'}
                      </p>
                    </div>
                  ) : EXPERIENCES.map(exp => (
                <div
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-5">
              {venuesLoading
                ...
                : venuesError ? (
                    <div className="col-span-2 bg-white rounded-2xl border border-dashed border-gray-200 p-8 text-center">
                      <p className="font-bold text-gray-800 text-[15px]">Venues could not load</p>
                      <p className="text-[12px] text-gray-400 mt-1">{venuesError}</p>
                    </div>
        ...
        <div className="mb-6">
          ...
          <div className="flex gap-3 overflow-x-auto no-scrollbar px-6 snap-x pb-1">
            {eventsLoading
              ...
              : EXPERIENCES.length === 0 ? (
                  <div className="snap-start shrink-0 w-60 bg-card rounded-2xl border border-dashed border-border p-5">
                    <p className="font-bold text-foreground text-[14px]">
                      {eventsError ? 'Events could not load' : 'No upcoming events yet'}
                    </p>
                    <p className="text-[12px] text-muted-foreground mt-1 leading-relaxed">
                      {eventsError ? eventsError : 'Live future-dated events will appear here.'}
                    </p>
                  </div>
                ) : EXPERIENCES.map(exp => (
              <div
          ...
          {venuesLoading
            ...
            : venuesError ? (
                <div className="bg-card rounded-3xl border border-dashed border-border p-6 text-center">
                  <p className="font-bold text-foreground text-[14px]">Venues could not load</p>
                  <p className="text-[12px] text-muted-foreground mt-1">{venuesError}</p>
                </div>
              ) : VENUES.map((venue, idx) => {
            const isGemLocked = venue.tier === 'Hidden Gem' && !paymentLinked;

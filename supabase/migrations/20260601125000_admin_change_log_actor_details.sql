create or replace function public.admin_get_venue_change_log(p_venue_id uuid)
returns table (
  id uuid,
  venue_id uuid,
  changed_by uuid,
...
begin
  if not public.is_admin_user() then
    raise exception 'Only admins can view venue change logs';
  end if;
  ...
  select
    ...
    p.display_name::text as actor_display_name,
    p.username::text as actor_username,
    vcl.field_name,
    vcl.old_value,
    vcl.new_value,
    vcl.risk_level,
    vcl.applied_immediately,
    vcl.created_reverification,
    vcl.reverification_reason,
    vcl.created_at
  from public.venue_change_log vcl
  left join public.profiles p on p.id = vcl.changed_by
  left join auth.users au on au.id = vcl.changed_by
  where vcl.venue_id = p_venue_id
...
as $$
declare
  target_venue_id uuid := venue_id;
  old_venue public.venues;
  updated_venue public.venues;
  notification_title text;
  notification_body text;
  review_reason text := nullif(trim(coalesce(reason, '')), '');
begin
  if not public.is_admin_user() then
  ...
  set
    ...
    reverification_reason = case
      ...
      when review_reason is not null then review_reason
  ...
  if new_status in ('needs_update', 'hidden') then
    ...
    values (target_venue_id, coalesce(review_reason, 'admin_review'), auth.uid());
  end if;
  ...
  values (
    ...
    auth.uid(),
    'listing_status',
    old_venue.listing_status,
    new_status,
    'high',
    true,
    new_status in ('needs_update', 'hidden'),
    coalesce(review_reason, case when new_status = 'live' then 'admin_approved' else null end)
  );
...
revoke all on function public.admin_get_venue_change_log(uuid) from public;
grant execute on function public.admin_get_venue_change_log(uuid) to authenticated;

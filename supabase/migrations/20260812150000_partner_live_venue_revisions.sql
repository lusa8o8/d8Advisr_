alter table public.venue_live_revisions
  add column revision_source text not null default 'admin'
  check (revision_source in ('admin', 'partner'));

create policy partner_view_own_live_revisions
on public.venue_live_revisions for select to authenticated
using (
  revision_source = 'partner'
  and exists (
    select 1 from public.venues v
    where v.id = venue_id and (
      v.partner_id = auth.uid()
      or (v.operator_organization_id is not null
        and public.is_active_organization_member(v.operator_organization_id, auth.uid())
        and public.organization_can(v.operator_organization_id, 'venues'))
    )
  )
);
...
begin
  ...
  if exists (select 1 from jsonb_object_keys(p_payload) field
    where field not in ('name','category','address','area','description','open_hours','cover_image','images')) then
  ...
  foreach key in array array['name','category','address','area','cover_image','images'] loop
  ...
  if proposed_values_value <> '{}'::jsonb and exists (
    select 1 from public.venue_live_revisions r where r.venue_id = p_venue_id and r.status = 'pending'
  ...
  next_hours := case when p_payload ? 'open_hours' then p_payload->'open_hours' else old_venue.open_hours end;
  if old_venue.description is distinct from next_description or old_venue.open_hours is distinct from next_hours then
    update public.venues set description = next_description, open_hours = next_hours, updated_at = now()
    where id = p_venue_id returning * into updated_venue;
    if old_venue.description is distinct from next_description then
      insert into public.venue_change_log (venue_id,changed_by,field_name,old_value,new_value,risk_level,applied_immediately,created_reverification,reverification_reason)
      values (p_venue_id,actor,'description',old_venue.description,next_description,'low',true,false,'partner_live_edit');
    end if;
    if old_venue.open_hours is distinct from next_hours then
      insert into public.venue_change_log (venue_id,changed_by,field_name,old_value,new_value,risk_level,applied_immediately,created_reverification,reverification_reason)
      values (p_venue_id,actor,'open_hours',old_venue.open_hours::text,next_hours::text,'low',true,false,'partner_live_edit');
    end if;
  else updated_venue := old_venue; end if;

  if proposed_values_value <> '{}'::jsonb then
    insert into public.venue_live_revisions (venue_id,previous_values,proposed_values,submitted_by,revision_source)
    values (p_venue_id,previous_values_value,proposed_values_value,actor,'partner')
    returning id into revision_id_value;
    insert into public.venue_reverification_tasks (venue_id,reason,status,triggered_by,notes,live_revision_id)
    values (p_venue_id,'partner_live_revision','open',actor,'Partner high-risk changes awaiting admin review',revision_id_value);
  end if;
  if revision_id_value is null and updated_venue.updated_at is not distinct from old_venue.updated_at then
    raise exception 'Live venue edit does not change any fields' using errcode = '22023';
  end if;
  return jsonb_build_object('venue_id',p_venue_id,'revision_id',revision_id_value,
    'pending_fields',coalesce((select jsonb_agg(k order by k) from jsonb_object_keys(proposed_values_value) k),'[]'::jsonb),
...
begin
  ...
  if decision='approved' then
    ...
    where id=venue.id;
    update public.venue_reverification_tasks set status='resolved',resolved_at=now(),notes=coalesce(nullif(btrim(p_note),''),notes)
    where live_revision_id=revision.id;
  else
    update public.venue_reverification_tasks set status='dismissed',resolved_at=now(),notes=coalesce(nullif(btrim(p_note),''),notes)
    where live_revision_id=revision.id;
  end if;
  update public.venue_live_revisions set status=decision,reviewed_by=actor,
    review_note=nullif(btrim(p_note),''),reviewed_at=now(),updated_at=now() where id=revision.id;

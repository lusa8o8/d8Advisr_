-- Add 'updated_live' to listing_admin_audit_log actions
...
alter table public.listing_admin_audit_log
  ...
  check (action in ('created', 'updated_draft', 'updated_live'));

create or replace function public.admin_update_live_event(
  p_event_id uuid,
  p_payload jsonb,
...
begin
  if not public.is_admin_user() then
    raise exception 'Only admins can edit live D8 events' using errcode = '42501';
  end if;
  ...
  if old_event.source is distinct from 'd8_admin'
    ...
    or old_event.event_status <> 'live'
  then
    raise exception 'Only live D8-admin events can use this editor' using errcode = '42501';
  end if;
  ...
  -- Enforce Commercial Policy Constraints
  if coalesce(old_event.is_free, false) and not coalesce(next_is_free, false) then
    raise exception 'published_free_event_cannot_become_paid' using errcode = '22023';
  end if;
  if not coalesce(old_event.is_free, false) and not coalesce(next_is_free, false) and next_price_pp > old_event.price_pp then
    raise exception 'published_event_price_cannot_increase' using errcode = '22023';
  end if;
  if not coalesce(next_is_free, false) and next_price_pp <= 0 then
...
revoke all on function public.admin_update_live_event(uuid, jsonb, timestamptz) from public, anon;
grant execute on function public.admin_update_live_event(uuid, jsonb, timestamptz) to authenticated;
